"""Tests for the main transpiluj function and related transformations."""

import pytest
from zmije.main import transpiluj, validuj_zadna_anglicka_klicova_slova


class TestTranspile:
    """Tests for the main transpiluj function."""

    def test_transpiluj_simple_keywords(self):
        """Test transpilation of simple single-word Czech keywords."""
        code = "Pravdivá_hodnota = Pravda"
        result = transpiluj(code)
        assert "True" in result

    def test_transpiluj_multi_word_keywords(self):
        """Test transpilation of multi-word Czech keywords like 'právě když'."""
        code = 'právě když X > 0:\n    vytiskni("text")'
        result = transpiluj(code)
        assert "if" in result

    def test_transpiluj_decimal_separator_czech(self):
        """Test conversion of Czech decimal separator (comma) to dot."""
        code = "Číslo = 3,14"
        result = transpiluj(code)
        assert "3.14" in result

    def test_transpiluj_decimal_with_leading_zero(self):
        """Test decimal separator with leading zero."""
        code = "Malé = 0,5"
        result = transpiluj(code)
        assert "0.5" in result

    def test_transpiluj_list_separator(self):
        """Test conversion of semicolons to commas in lists."""
        code = "Seznam = [1; 2; 3]"
        result = transpiluj(code)
        assert "[1,2,3]" in result.replace(" ", "")

    def test_transpiluj_czech_quotes(self):
        """Test conversion of Czech quotes to regular quotes."""
        code = 'Zpráva = "Ahoj světe"'
        result = transpiluj(code)
        assert '"Ahoj' in result

    def test_transpiluj_dict_with_semicolon_separator(self):
    ...
    def test_transpiluj_keyword_import(self):
    ...
    def test_transpiluj_keyword_function_def(self):
    ...
    def test_transpiluj_try_except(self):
    ...
    def test_transpiluj_class_definition(self):
        """Test transpilation of class definitions."""
        code = "klasa Zvire:\n    def __init__(self; Jméno):\n        self.Jméno = Jméno"
        result = transpiluj(code)
        assert "class" in result

    def test_transpiluj_for_loop(self):
    ...
    def test_transpiluj_while_loop(self):
        """Test transpilation of while loops."""
        code = "při X < 5:\n    vytiskni(X)"
        result = transpiluj(code)
        assert "while" in result

    def test_transpiluj_delete_keyword(self):
        """Test transpilation of delete statement."""
        code = "smaž x"
        result = transpiluj(code)
        assert "del" in result

    def test_transpiluj_yield_keyword(self):
        """Test transpilation of yield keyword."""
        code = "def generátor():\n    vynes 1"
        result = transpiluj(code)
        assert "yield" in result

    def test_transpiluj_with_statement(self):
    ...
    def test_transpiluj_complex_expression(self):
    ...
    def test_transpiluj_output_is_valid_python(self):
        """Test that transpilujd code is valid Python."""
        code = 'X = Pravda\nY = 3,14\nZpráva = "Ahoj"'
        result = transpiluj(code)
        # This should not raise an exception
        compile(result, '<test>', 'exec')

    def test_transpiluj_preserves_indentation(self):
    ...
    def test_transpiluj_decimal_in_function_call(self):
...
class TestValidateNoEnglishKeywords:
    ...
    def test_validate_rejects_english_for(self):
        ...
        with pytest.raises(ValueError, match="for"):
            validuj_zadna_anglicka_klicova_slova("for i in range(10):\n    pass")
    ...
    def test_validate_rejects_english_print(self):
        ...
        with pytest.raises(ValueError, match="print"):
            validuj_zadna_anglicka_klicova_slova("print('hello')")

    def test_validate_rejects_english_class(self):
        """Test that English 'class' keyword is rejected."""
        with pytest.raises(ValueError, match="class"):
            validuj_zadna_anglicka_klicova_slova("class MyClass:\n    pass")
    ...
    def test_validate_error_message_includes_line_number(self):
        ...
        with pytest.raises(ValueError, match="řádku"):
            validuj_zadna_anglicka_klicova_slova("x = 5\nif y > 0:\n    pass")
...
class TestTranspileEdgeCases:
    ...
    def test_transpiluj_empty_code(self):
        """Test transpilation of empty code."""
        code = ""
        result = transpiluj(code)
        assert result is not None

    def test_transpiluj_only_comments(self):
        """Test transpilation of code with only comments."""
        code = "# Komentář 1\n# Komentář 2"
        result = transpiluj(code)
        assert "Komentář" in result

    def test_transpiluj_whitespace_handling(self):
        """Test that transpilation handles various whitespace."""
        code = "právě   když   X > 0:\n    vytiskni(„ok\")"
        result = transpiluj(code)
        assert "if" in result

    def test_transpiluj_mixed_czech_quotes(self):
    ...
    def test_transpiluj_consecutive_decimals(self):
